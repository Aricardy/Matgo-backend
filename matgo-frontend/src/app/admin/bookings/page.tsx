            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const formattedBookings = bookingsData.map((booking: any) => ({
                        id: booking.id,
                        tripId: booking.tripId || booking.bookingId || booking.id,
                        seat: booking.seats || booking.seat || 'N/A',
                        status: booking.status === 'confirmed' ? 'Confirmed' : 
                               booking.status === 'pending' ? 'Pending' : 'Confirmed',
                        paymentStatus: booking.paymentStatus === 'paid' || booking.isPaid ? 'Paid' : 'Unpaid',
                        departure: booking.departure || booking.origin || 'Unknown',
                        destination: booking.destination || booking.route || 'Unknown',
                        busName: booking.busName || booking.vehicleInfo || booking.vehicle || 'Unknown Bus'
                    }));
