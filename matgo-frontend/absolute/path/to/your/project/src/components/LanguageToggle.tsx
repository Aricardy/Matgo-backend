  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" aria-label={`Change language, current: ${language === 'ENG' ? 'English' : 'Kiswahili'}`}>
          <Languages className="h-5 w-5" />
           <span className="sr-only">Change language, current: {language === 'ENG' ? 'English' : 'Kiswahili'}</span>
        </Button>
      </DropdownMenuTrigger>
