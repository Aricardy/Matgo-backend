      ...
      <main className="flex flex-1 flex-col items-center justify-center p-6 text-center relative z-10">
        ...
        <div className="mt-12 flex flex-col gap-4 sm:flex-row sm:justify-center animate-fade-in animation-delay-[1300ms]">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="lg" className="font-bold text-lg px-10 py-7 transition-transform hover:scale-105 btn-glow-primary">
                {currentContent.signUp} <ChevronDown className="ml-2 h-5 w-5"/>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-56 glassy-card border-primary/30">
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link href="/signup?role=passenger" className="font-semibold text-base py-2.5 flex items-center gap-2"><User/>{currentContent.passenger}</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link href="/signup?role=crew" className="font-semibold text-base py-2.5 flex items-center gap-2"><Briefcase/>{currentContent.crew}</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link href="/signup?role=sacco" className="font-semibold text-base py-2.5 flex items-center gap-2"><Building/>{currentContent.sacco}</Link>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full animate-fade-in animation-delay-[1500ms]">
          {[
            ...
            <div key={feature.title} className="p-6 rounded-xl glassy-card text-card-foreground transform transition-all hover:shadow-primary/30 hover:shadow-2xl hover:-translate-y-1.5">
              <h3 className="font-headline text-2xl text-primary mb-2">{feature.title}</h3>
